...
from aidedecamp.orchestrator.grants import parse_action, parse_domain, parse_rung
...
def test_build_app_loads_persisted_matrix(tmp_path):
    ...
    from aidedecamp.app import build_app
    ...
    settings = Settings.from_env({
        "ADC_MEM0_URL": "", "ADC_AUTONOMY_STATE_PATH": grants_path,
        "ADC_AUDIT_LOG_PATH": str(tmp_path / "audit.jsonl"),
    })
...
def test_graduation_suggested_when_bar_met(tmp_path):
    ...
    assert "aidedecamp autonomy grant draft_reply mail act_notify" in rendered
...
from aidedecamp.cli.autonomy_cmd import (  # noqa: E402
    run_autonomy_grant,
    run_autonomy_record,
    run_autonomy_revoke,
    run_autonomy_show,
)
from aidedecamp.config import Settings  # noqa: E402


def _settings(tmp_path):
    return Settings.from_env({
        "ADC_MEM0_URL": "",
        "ADC_AUTONOMY_STATE_PATH": str(tmp_path / "grants.json"),
        "ADC_AUDIT_LOG_PATH": str(tmp_path / "audit.jsonl"),
    })
...
def test_chat_autonomy_command_shows_posture_never_grants(tmp_path):
    ...
    from aidedecamp.dispatcher import handle_slack_message
...
def test_send_gate_survives_send_reply_grant():
    ...
    from aidedecamp.connectors.base import SendNotPermitted
    from aidedecamp.connectors.direct_oauth import DirectOAuthConnector
