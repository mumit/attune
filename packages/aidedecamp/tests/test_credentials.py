...
def _build_google_mocks():
    ...
    adc_creds = _FakeADCCreds()
    auth_mod.default = MagicMock(return_value=(adc_creds, "project"))
    ...
    return google_mod, auth_mod, sa_mod, user_creds_mod, adc_creds


@pytest.fixture()
def google_mocks(tmp_path, monkeypatch):
    """Inject fake google modules and yield (sa_mod, user_creds_mod, adc_creds, tmp_path)."""
    google_mod, auth_mod, sa_mod, user_creds_mod, adc_creds = _build_google_mocks()
    ...
    with patch.dict(sys.modules, patches):
        ...
        if "aidedecamp.credentials" in sys.modules:
            del sys.modules["aidedecamp.credentials"]
        yield sa_mod, user_creds_mod, adc_creds, tmp_path
...
def test_service_account_creds_loaded(google_mocks):
    ...
    from aidedecamp.credentials import SCOPES_DEFAULT, load_google_credentials
    from aidedecamp.config import Settings

    settings = Settings.from_env(
        {"ADC_DEPLOYMENT": "personal", "ADC_CONNECTOR_MODE": "mcp",
         "ADC_MEM0_URL": "", "ADC_AUDIT_LOG_PATH": "",
         "ADC_GOOGLE_CREDENTIALS_FILE": str(cred_file)}
    )
...
def test_service_account_custom_scopes(google_mocks):
    ...
    from aidedecamp.credentials import load_google_credentials
    from aidedecamp.config import Settings
...
def test_adc_fallback_used_when_no_file(google_mocks):
    _, _, adc_creds, _ = google_mocks
    from aidedecamp.credentials import load_google_credentials, SCOPES_DEFAULT
    from aidedecamp.config import Settings

    settings = Settings.from_env(
        {"ADC_DEPLOYMENT": "personal", "ADC_CONNECTOR_MODE": "mcp",
         "ADC_MEM0_URL": "", "ADC_AUDIT_LOG_PATH": ""}
        # no ADC_GOOGLE_CREDENTIALS_FILE
    )
    creds = load_google_credentials(settings)
    assert creds is adc_creds


def test_adc_receives_default_scopes(google_mocks):
...
def test_missing_credentials_file_raises(google_mocks):
    ...
    settings = Settings.from_env(
        {"ADC_DEPLOYMENT": "personal", "ADC_CONNECTOR_MODE": "mcp",
         "ADC_MEM0_URL": "", "ADC_AUDIT_LOG_PATH": "",
         "ADC_GOOGLE_CREDENTIALS_FILE": str(tmp_path / "nonexistent.json")}
    )
