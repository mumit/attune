...
from .credentials import load_google_chat_credentials, load_google_credentials
...
class Runtime:
    ...
    def process_chat_event(self, event: dict[str, Any]) -> None:
        ...
        if self.gchat is None or "google_chat" not in self.settings.interaction_channels:
            return
    ...
    def process_chat_delivery(self, event: dict[str, Any]) -> None:
        """Route a verified Google Chat app webhook delivery.

        Message events and card actions share Google's synchronous HTTP
        endpoint.  The stateless republisher places both on the interaction
        topic; this method converts app ``MESSAGE`` events to the same shape
        used by Workspace Events and sends card clicks to the approval path.
        """
        if event.get("type") == "MESSAGE":
            message = dict(event.get("message") or {})
            if "space" not in message and event.get("space"):
                message["space"] = event["space"]
            if "sender" not in message and event.get("user"):
                message["sender"] = event["user"]
            self.process_chat_event({
    ...
    def _notify_all(self, text: str) -> None:
        ...
        if "slack" in self.settings.notification_channels and self.slack_say is not None:
    ...
    def build_scheduler(self) -> Any:
        ...
        has_brief_destination = (
            ...
            "google_chat" in self.settings.brief_channels
            and self.gchat is not None and self.settings.chat_default_space
        )
        has_notification_destination = (
            "slack" in self.settings.notification_channels
            and self.slack_say is not None
        ) or (
            "google_chat" in self.settings.notification_channels
            and self.gchat is not None and self.settings.chat_default_space
        )
    ...
    def poll_once(self) -> dict[str, Any]:
        ...
        if self.gmail_service is None:
            try:
                threads, events = poll_workspace_connector(
                    self.connector, self.connector_poll_state
                )
                for thread in threads:
                    submit_gmail_thread(
                        self.app, thread, gmail_tid=thread.thread_id,
                        history_id="connector-poll",
                        post_approval=self._post_mail_approval,
                        user_id=self.settings.user_id,
                        audit_log=self.app.audit_log, pending=self.pending,
                        notify=self._notify_all,
    ...
    def run_poll_loop(self) -> None:  # pragma: no cover - thin timer shell
        """Tick poll_once every ATTUNE_POLL_SECONDS, backing off on whole-tick
...
def build_runtime(
    ...
    *,
    ...
    - *gmail_service* / *calendar_service* — raw Gmail/Calendar API resources,
      built only for the Google OAuth backend
    ...
    if settings.workspace_backend == WorkspaceBackend.GOOGLE_OAUTH and resolved_credentials is None and (
    ...
    if resolved_gmail_service is None and settings.workspace_backend == WorkspaceBackend.GOOGLE_OAUTH:  # pragma: no cover - requires live creds
    ...
    if resolved_calendar_service is None and settings.workspace_backend == WorkspaceBackend.GOOGLE_OAUTH:  # pragma: no cover - requires live creds
        from googleapiclient.discovery import build as _build
    ...
    channel_uses_slack = bool(
        "slack" in settings.brief_channels
        or settings.approval_channel == "slack"
        or "slack" in settings.notification_channels
        or "slack" in settings.interaction_channels
    )
    if resolved_slack is None and settings.slack_bot_token and channel_uses_slack:
    ...
    channel_uses_chat = bool(
        "google_chat" in settings.brief_channels
        or settings.approval_channel == "google_chat"
        or "google_chat" in settings.notification_channels
        or "google_chat" in settings.interaction_channels
    )
    ...
    if resolved_chat_events_service is None and resolved_credentials is not None:  # pragma: no cover - requires live creds
        from googleapiclient.discovery import build as _build
