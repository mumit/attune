...
1. **Persistence.** `JsonPermissionMatrixStore(path)` (the
   ...
   (`ADC_AUTONOMY_STATE_PATH`, data-dir-derived). `app.build_app` loads the
...
3. **Surfaces.** CLI `aidedecamp autonomy` group: `show`, `grant <action>
...
4. **Weekly digest job** (scheduler, prompt 05): post track-record
   ...
   act-and-notify, run: `aidedecamp autonomy grant …`").
