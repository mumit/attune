...
Today the only entrypoint is `python -m attune` (which immediately needs
...
Build an `attune` console-script CLI (stdlib `argparse` — no click/typer
dependency) in `src/attune/cli/`, wired via
`[project.scripts]`. Subcommands:

1. **`attune init`** — interactive setup wizard writing `.env` (never
   committing it; refuse to overwrite without `--force`):
   - asks workspace backend, ingestion transport, and principal identity
   ...
   - configured OpenAI-compatible gateway token (input hidden via `getpass`, written to `.env` only),
   ...
   - a single **data directory** (default `~/.attune/`): add
     `Settings.data_dir` (`ATTUNE_DATA_DIR`) and make every existing `*_path`
...
2. **`attune doctor`** — read-only validation, one PASS/FAIL/SKIP line
   per check with a *specific* fix hint: env parse, data dir writable,
   configured OpenAI-compatible gateway reachability (one cheap models/completions call; a 401 must
...
3. **`attune brief`** — assemble one brief and print it to stdout
   (`--post` additionally posts to configured channels). The "try it in a
   terminal before wiring any chat app" moment.
4. **`attune run`** — `build_runtime().run()` after a fast doctor pass
   of the fatal checks (skippable with `--no-checks`).
5. Reserve `attune memory` and `attune autonomy` as subcommand
   groups that print "coming in M4" (prompts 11/12 fill them).

## Constraints

- Lazy-import heavy deps inside subcommands so `attune --help` works in
  a bare install.
...
- README + `docs/deployment.md` gain a short "or just run `attune
