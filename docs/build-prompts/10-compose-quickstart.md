...
creation. The happy path deserves to be: clone → compose up → `aidedecamp
init` → `aidedecamp brief`.

## Task

1. **`deploy/compose.yml`** — the full local/VM stack: Qdrant, Mem0, and the
   assistant itself (a new `Dockerfile` for the aidedecamp package,
   `python -m aidedecamp` entrypoint, `.env` file passed via `env_file`,
...
2. **Root `README.md` quickstart rewrite** — a genuinely ~15-minute path:
   prerequisites (Docker, a Google account, a Fuel iX token), four steps
   (clone, `docker compose up -d` for the substrate, `pip install -e` +
   `aidedecamp init`, `aidedecamp brief`), then "make it always-on"
   (`aidedecamp run` under the compose assistant profile or systemd), then
...
- `docker compose -f deploy/compose.yml config` validates in CI-less local
  check; the aidedecamp image builds (`docker build`) — include both as a
