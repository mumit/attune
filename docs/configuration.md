...
| `ATTUNE_APPROVAL_EXPIRY_DAYS` | `7` | Days after which an unanswered (or already-ignored) approval card's underlying workflow is cancelled outright (build prompt 31) — distinct from the ignore threshold above, which only records a signal; this releases the LangGraph checkpoint, so a later click gets an honest "this proposal expired" refusal rather than resuming. See `docs/decisions.md` for the 7-day justification. |
| `ATTUNE_ROUTINE_STATE_PATH` | `ATTUNE_DATA_DIR/routines.json` | Path to the user-authored routines store (build prompt 32) — the daily brief ships as one default routine here (seeded on first touch), editable and removable via `attune routine`. |
| `ATTUNE_MCP_SERVER_PROPOSALS_STATE_PATH` | `ATTUNE_DATA_DIR/mcp_proposals.json` | Path to the durable store for MCP-originated `attune.propose` tasks (build prompt 34) — a calling agent's proposal sits here in an input-required state until resolved via `attune mcp-server proposals approve/reject`. |
| `ATTUNE_DAILY_ATTENTION_BUDGET` | `5` | The global daily cap on unsolicited proactive messages across every feature combined (label/hold/decline proposals, follow-up nudges, routines) — build prompt 32. URGENT items always bypass it. See `docs/decisions.md` for the default's basis. |
| `ATTUNE_ATTENTION_BUDGET_DB_PATH` | `ATTUNE_DATA_DIR/attention_budget.db` | Path to the shared daily attention-budget spend counter (build prompt 32) — one row per calendar day, decremented as each proactive feature spends from today's budget. |
...
| `ATTUNE_LEDGER_DB_PATH` | `ATTUNE_DATA_DIR/decision_ledger.db` | Path to the decision-ledger SQLite database (build prompt 26): one row per proposal, with context attribution, edit metrics, and autonomy fields, aggregated by `attune metrics` into the north-star edit-burden number and its mandatory coverage denominator. Analytic state, not the trust root — the audit log (`ATTUNE_AUDIT_LOG_PATH`) stays authoritative for authority decisions. |
| `ATTUNE_SCHEDULER_DB_PATH` | `ATTUNE_DATA_DIR/scheduler.db` | Path to the durable scheduler's job SQLite database (build prompt 32): next-run, last-run outcome, and last error per recurring job, so a restart resumes exactly where the last process left off (with per-job catch-up semantics) instead of forgetting every job existed. Read by `attune status` and Doctor to surface a silently failing recurring job. |
| `ATTUNE_PLAYBOOK_DIR` | `ATTUNE_DATA_DIR/playbook` | Directory for the git-backed playbook (build prompt 29): one Markdown file per domain (mail/calendar/voice/scheduling), each its own git repository, written only by the nightly reflector via bounded delta edits (add/refine/retire). Inspect and correct it with `attune playbook`. Grants no authority — shapes tone/phrasing only. |
| `ATTUNE_EVAL_CASES_DIR` | `./evals/cases` | Directory of redacted regression cases written by `attune eval capture` and read by `attune eval run`/`attune eval label` (build prompt 27). A checked-in project fixture directory, not `ATTUNE_DATA_DIR`-relative instance state. |
| `ATTUNE_EVAL_TRIAGE_CASES_PATH` | `./evals/triage_cases.json` | Path to the hand-labelled triage regression set (build prompt 27): expected URGENT/ROUTINE/NOISE priority plus, where recorded, the expected importance-adjustment direction. |
