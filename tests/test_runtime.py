...
class _FakeMemoryStore:
    ...
    def consolidate(self, *, user_id):
        from attune.memory.base import ConsolidationReport
...
def _settings(**overrides):
    base = {
        ...
        "ATTUNE_CHAT_ALLOWED_USERS": "users/U1,users/1,users/tester",
...
def test_build_runtime_mcp_backend_does_not_load_google_user_credentials():
    settings = _settings(
        ATTUNE_WORKSPACE_BACKEND="mcp",
        ATTUNE_MCP_URL="https://mcp.example/mcp",
        ATTUNE_BRIEF_CHANNELS="",
        ATTUNE_APPROVAL_CHANNEL="",
        ATTUNE_NOTIFICATION_CHANNELS="",
        ATTUNE_INTERACTION_CHANNELS="",
    )
    connector = _FakeConnector()

    with patch(
        "attune.runtime.load_google_credentials",
        side_effect=AssertionError("MCP must not load Google user credentials"),
    ):
        runtime = build_runtime(settings, app=_app_ctx(), connector=connector)

    assert runtime.connector is connector
    assert runtime.gmail_service is None
    assert runtime.calendar_service is None
...
def test_build_runtime_defaults_watch_and_chat_state_to_json_files(tmp_path):
    ...
    settings = _settings(
        ATTUNE_GMAIL_WATCH_STATE_PATH=str(watch_path),
        ATTUNE_CHAT_SUBSCRIPTION_STATE_PATH=str(chat_path),
    )
...
def test_build_runtime_builds_slack_channel_when_bot_token_present():
    ...
    from attune.channels import SlackChannel
...
def test_build_runtime_builds_slack_say_when_channel_configured():
    settings = _settings(SLACK_BOT_TOKEN="xoxb-token", ATTUNE_SLACK_CHANNEL="C123")
    runtime = build_runtime(
        settings, app=_app_ctx(), connector=_FakeConnector(),
        gmail_service=_FakeGmailService(), chat_events_service=object(), calendar_service=object(),
    )
...
def test_slack_actor_uses_canonical_principal_memory_id():
    ...
    store = _Store()
    settings = _settings(
        SLACK_BOT_TOKEN="xoxb-token", ATTUNE_USER_ID="owner@example.com"
    )
...
def test_build_runtime_gchat_none_when_no_default_space():
    settings = _settings()  # no ATTUNE_CHAT_SPACE
    runtime = build_runtime(
        settings, app=_app_ctx(), connector=_FakeConnector(),
        gmail_service=_FakeGmailService(), chat_events_service=object(), calendar_service=object(),
    )
...
def test_build_runtime_respects_gchat_override_even_with_default_space():
    ...
    settings = _settings(ATTUNE_CHAT_SPACE="spaces/ABC")
...
def test_build_runtime_builds_gchat_via_make_chat_send_fn(tmp_path):
    ...
    settings = _settings(
        ...
        ATTUNE_CHAT_CREDENTIALS_FILE=str(tmp_path / "chat.json"),
    )
    with patch("attune.runtime.load_google_chat_credentials", return_value=object()), patch.dict(
        sys.modules,
    ...
    from attune.channels import GoogleChatChannel
...
def test_process_gmail_notification_posts_to_single_approval_channel():
    slack = _FakeSlackChannel()
    gchat = _FakeGChatChannel()
...
def test_process_gmail_notification_gchat_only_when_no_slack():
    ...
    runtime.settings = _settings(
        ATTUNE_CHAT_SPACE="spaces/ABC", ATTUNE_APPROVAL_CHANNEL="google_chat"
    )
...
def test_process_gmail_notification_uses_settings_user_id():
    ...
    runtime.settings = _settings(ATTUNE_USER_ID="someone@example.com")
...
def test_chat_app_message_delivery_uses_google_chat_interaction_path():
    gchat = _FakeGChatChannel()
    client = _FakeClient(reply="reply from Attune")
    runtime = _runtime(gchat=gchat, app=_app_ctx(client=client))
    runtime.settings = _settings(ATTUNE_CHAT_SPACE="spaces/ABC")

    runtime.process_chat_delivery({
        "type": "MESSAGE",
        "message": {
            "text": "hello",
            "sender": {"name": "users/U1", "type": "HUMAN"},
    ...
    assert gchat.texts == [("spaces/ABC", "reply from Attune")]
...
def test_renew_gmail_watch_uses_settings_user_id_and_topic():
    ...
    runtime.settings = _settings(
        ATTUNE_USER_ID="custom@example.com", ATTUNE_GMAIL_PUBSUB_TOPIC="projects/p/topics/t"
    )
...
def test_renew_chat_subscription_uses_settings_space_and_topic():
    ...
    runtime.settings = _settings(
        ATTUNE_CHAT_SPACE="spaces/ABC", ATTUNE_CHAT_PUBSUB_TOPIC="projects/p/topics/chat"
    )
...
def test_build_runtime_defaults_calendar_state_to_json_files(tmp_path):
    ...
    settings = _settings(
        ATTUNE_CALENDAR_WATCH_STATE_PATH=str(watch_path),
        ATTUNE_CALENDAR_SYNC_STATE_PATH=str(sync_path),
    )
...
def test_process_calendar_notification_reconciles_with_baseline():
    ...
    runtime.settings = _settings(ATTUNE_CALENDAR_ID="primary")
...
def test_renew_calendar_watch_uses_settings_calendar_id_and_address():
    ...
    runtime.settings = _settings(
        ...
        ATTUNE_CALENDAR_WEBHOOK_ADDRESS="https://republisher.example.com/hook",
    )
...
def test_process_chat_interaction_noop_when_gchat_none():
    runtime = _runtime(gchat=None)
    # Must not raise even though there's no channel to post to.
    runtime.process_chat_interaction(_interaction_click("attune_approve", "t-1"))


def test_process_chat_interaction_noop_when_route_disabled():
    graph = _FakeGraph()
    runtime = _runtime(app=_app_ctx(graph=graph), gchat=_FakeGChatChannel())
    runtime.settings = _settings(
        ATTUNE_CHAT_SPACE="spaces/ABC", ATTUNE_INTERACTION_CHANNELS="slack"
    )
...
def test_process_chat_interaction_resumes_and_posts_confirmation():
    ...
    runtime.process_chat_interaction(_interaction_click("attune_approve", "t-42"))
...
def test_process_chat_interaction_reject_posts_rejection_confirmation():
    ...
    runtime.process_chat_interaction(_interaction_click("attune_reject", "t-9"))
...
def test_failure_log_never_contains_payload(caplog):
    ...
    runtime = _runtime()
    secret_body = b'{"token": "SECRET-configured gateway-TOKEN-VALUE"}'
...
def test_gmail_handler_renews_on_history_expired():
    from attune.ingestion import HistoryExpired
...
def test_next_backoff_doubles_and_caps():
    from attune.runtime import next_backoff
...
def test_loop_stats_heartbeat_cadence_and_reset():
    from attune.runtime import LoopStats
...
def test_logging_setup_json_mode_emits_json_lines():
    ...
    from attune.logging_setup import JsonFormatter

    record = _logging.LogRecord(
        name="attune.runtime", level=_logging.INFO, pathname=__file__,
        lineno=1, msg="heartbeat gmail: pulled=%d", args=(3,), exc_info=None,
    )
    entry = _json.loads(JsonFormatter().format(record))
    assert entry["level"] == "INFO"
    assert entry["logger"] == "attune.runtime"
...
def test_logging_setup_configure_is_idempotent():
    ...
    from attune.logging_setup import configure
...
def test_build_scheduler_assembles_expected_jobs():
    ...
    nudging = _runtime(slack=_FakeSlackChannel(), slack_say=lambda **kw: None,
                       nudge_state=object())
    nudging.settings = _settings(ATTUNE_USER_ID="me@example.com")
...
def test_renew_all_watches_isolates_and_audits_failures():
    ...
    runtime.settings = _settings(
        ...
        ATTUNE_CHAT_PUBSUB_TOPIC="projects/p/topics/chat",
        ATTUNE_CHAT_SPACE="spaces/ABC",
    )
...
def test_weekly_autonomy_digest_posts_suggestions_to_channels():
    ...
    assert "attune autonomy grant" in said[0]["text"]
    assert len(gchat.texts) == 1
...
def test_process_chat_interaction_ignores_edit():
    ...
    runtime.process_chat_interaction(_interaction_click("attune_edit", "t-1"))
