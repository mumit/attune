...
from attune.connectors.google_oauth import (
...
from attune.connectors.base import Provenance, SendNotPermitted
...
_GOOGLE_EVENT = {
    ...
    "attendees": [{"email": "alice@external.test"}, {"email": "bob@example.com"}],
}
...
def test_list_events_flags_external_attendees():
    ...
    conn = _conn(calendar=cal)
    events = conn.list_events(time_min=_NOW, time_max=_LATER)
    # alice@external.test is outside the configured example.com domain.
