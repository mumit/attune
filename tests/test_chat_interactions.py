...
from attune.ingestion.chat_interactions import ChatInteraction, decode_chat_interaction
...
def test_approve_decodes_to_approved():
    result = decode_chat_interaction(_click("attune_approve", "t-42"))
...
def test_reject_decodes_to_rejected():
    result = decode_chat_interaction(_click("attune_reject", "t-9"))
...
def test_edit_returns_none():
    ...
    result = decode_chat_interaction(_click("attune_edit", "t-1"))
...
def test_missing_thread_id_returns_none():
    result = decode_chat_interaction(_click("attune_approve", thread_id=None))
    assert result is None
...
def _edit_submit(text: str | None, thread_id: str = "t-5") -> dict:
    event = _click("attune_edit_submit", thread_id)
    if text is not None:
        event["common"] = {
            "formInputs": {
                "attune_edit_text": {"stringInputs": {"value": [text]}}
            }
        }
    return event
...
def test_approve_and_reject_carry_no_text():
    assert decode_chat_interaction(_click("attune_approve", "t-1")).text is None
    assert decode_chat_interaction(_click("attune_reject", "t-1")).text is None
...
def test_action_names_match_blocks_py():
    ...
    from attune.ingestion.chat_interactions import (
        _ACTION_APPROVE,
        _ACTION_EDIT_SUBMIT,
        _ACTION_REJECT,
    )
...
def test_dialog_field_matches_gchat_cards():
    from attune.channels.gchat_cards import EDIT_DIALOG_FIELD
    from attune.ingestion.chat_interactions import _EDIT_DIALOG_FIELD
