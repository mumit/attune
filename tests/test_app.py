...
a FakeStore, and the configured OpenAI-compatible gateway client by a FakeClient. The real wiring logic —
...
from attune.app import AppContext, build_app
...
def test_build_app_constructs_real_audit_log_when_absent(tmp_path):
    ...
    settings = Settings.from_env({"ATTUNE_AUDIT_LOG_PATH": str(log_path)})
    app = build_app(
        settings=settings,
        client=FakeClient(),
        store=FakeStore(),
...
def test_build_app_uses_provided_settings():
    settings = Settings.from_env({"ATTUNE_WORKSPACE_BACKEND": "mcp", "ATTUNE_MCP_URL": "https://mcp.example"})
    app = _fake_app(settings=settings)
    assert app.settings.workspace_backend == WorkspaceBackend.MCP
